...
public class PracticeMeasureCountR extends ProviderLocationCount{
	...
	public boolean verify(String elementmethod,String locator,String data)  
	{
		...
		String[] words=uiCount.split(" ");
		
		
		Setup.log.info("Ui_Count"+words[3]);
		Setup.log.info("Db_Count"+dbCount);
		
		// For time been we are set hardcode value to match with UI instead of Query
		// As par discuss with awadhesh having problem with DB Table
		String hardcodeDBCount = "20";
		Setup.driver.navigate().refresh();
		if(words[3].equals(hardcodeDBCount))
		{
